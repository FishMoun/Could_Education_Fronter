    ...
    <el-header>
      <div class="searchbox">
        <el-select v-model="termValue" placeholder="课程选择" style="margin-right: 10px">
          <el-option v-for="item in termoptions" :key="item.courseId" :label="item.courseName" :value="item.courseId" />
        </el-select>
          ...
          <div class="addHomework">
            ...
            <div v-for="(item, index) in form.homeworks">
              <el-form-item :label="`题目${index + 1}`">
                <el-input v-model="item.context" :rows="4" type="textarea" resize="none" />
              </el-form-item>
              <el-form-item label="参考答案:">
                <el-input v-model="item.refAnswer" :rows="2" type="textarea" resize="none" />
              </el-form-item>
              <el-button style="float: right;" @click="addProblem"
                v-if="index + 1 === form.homeworks.length">+添加题目</el-button>
            </div>
          </div>
        </el-form>
        <template #footer>
